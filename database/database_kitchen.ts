...
export const get_kitchen_content_d = async (request: string) => {
	...
	try {
        ...
        WITH f, in_rel, g
        ORDER BY g.name
        WITH f, collect(CASE WHEN in_rel IS NOT NULL THEN { id: g.id, name: g.name, expires: in_rel.expires, purchased: in_rel.purchased, has_id: in_rel.id } ELSE NULL END) AS groceries
        RETURN f.id AS kitchen_id, $kitchen_name AS kitchen_name, $owner AS isOwner, f.location AS location, 
        CASE WHEN size(groceries) > 0 THEN groceries ELSE [] END AS groceries
			...
			console.log(kitchen.records[0].toObject());
