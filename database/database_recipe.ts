...
export const find_recipes_by_kitchen_d = async (request: string) => {
  ...
  try {
    const result = await session.run(
      ...
      WITH r, kitchenGroceries, [g IN recipeGroceries WHERE g IN kitchenGroceries] AS matchingIngredients, [g IN recipeGroceries WHERE NOT g IN kitchenGroceries] AS missingIngredients
      WHERE size(matchingIngredients) >= 3
    ...
    return result.records.map((record) => {
      ...
      return {
        ...
        missingIngredients: missingIngredients.map(
          (grocery: { properties: { id: any; name: any } }) => ({
            id: grocery.properties.id,
            name: grocery.properties.name,
          })
        ),
      };
    });
...
export const find_recipes_by_groceries_d = async (request: string[]) => {
  ...
  try {
    const result = await session.run(
      ...
      WHERE size([g IN groceries WHERE g.hasGrocery]) >= 1
